...
The examples below parameterize the model as `${MODEL_NAME}` and the proxy endpoint as `${IP}`. Set `${MODEL_NAME}` to [`Qwen/Qwen3-VL-32B-Instruct`](https://huggingface.co/Qwen/Qwen3-VL-32B-Instruct) from the [multimodal aggregation guide](https://github.com/llm-d/llm-d/tree/main/guides/multimodal-serving/aggregation), and set `${IP}` to the proxy endpoint IP retrieved per that guide's verification steps.
